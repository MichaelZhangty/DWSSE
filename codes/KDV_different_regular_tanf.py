...
class DeepHPM:
    ...
    def idn_init(self, t, x, u, u_layers, pde_layers):
        ...
        self.idn_u_pred = self.idn_net_u(self.t_tf, self.x_tf)[0]
        self.idn_regular_pred = self.idn_net_u(self.t_tf, self.x_tf)[1]
        self.pde_pred = self.net_pde(self.terms_tf)
        self.idn_f_pred = self.idn_net_f(self.t_tf, self.x_tf)

        # loss for Identification
        self.regular_weights = tf.reduce_sum(self.one_tf)
        # self.idn_u_loss = tf.reduce_sum(tf.square(self.idn_u_pred - self.u_tf))
        self.idn_u_loss = tf.reduce_sum(tf.square(self.idn_u_pred - self.u_tf)) + tf.reduce_sum(tf.square(self.idn_regular_pred))/2/self.regular_weights
        # self.idn_f_loss = tf.reduce_sum(tf.square(self.idn_f_pred))
        self.idn_f_loss = tf.reduce_sum(
            tf.multiply(tf.subtract(self.one_tf, tf.nn.tanh(tf.square(self.idn_u_pred - self.u_tf))),
                        tf.square(self.idn_f_pred)))
